...
cd "${workingdir}/outs/"
apptainer run -B /data "\$container" cellranger multi --id "${library_id}" --csv "${library_csv}" --localcores "\$num_cores"
...
mkdir -p cellbender
container="${HOME}/scratch/tmp/oscar-qc_latest.sif"
apptainer run --nv -B /data "\$container" cellbender remove-background --cuda --input outs/multi/count/raw_feature_bc_matrix.h5 --output cellbender/output.h5
rm ckpt.tar.gz
EOF
    fi
done
